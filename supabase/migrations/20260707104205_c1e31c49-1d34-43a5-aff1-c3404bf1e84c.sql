...
-- C4-fix: omzet fallback naar Eitje wanneer Lightspeed leeg is

-- 1. Samenvatting: omzet_effectief + omzet_bron_mix
...
BEGIN
  ...
  WITH per_day AS (
    SELECT
      ...
      u.loonkosten_bron,
      CASE WHEN COALESCE(d.omzet_incl, 0) > 0 THEN d.omzet_incl::numeric
           ELSE COALESCE(u.eitje_omzet_dag, 0)::numeric END AS omzet_eff,
      CASE WHEN COALESCE(d.omzet_incl, 0) > 0 THEN 'lightspeed'
           WHEN COALESCE(u.eitje_omzet_dag, 0) > 0 THEN 'eitje'
           ELSE 'geen' END AS omzet_bron
  ...
  SELECT jsonb_build_object(
    ...
    'totaal', jsonb_build_object(
      ...
      'omzet_bron_mix', jsonb_build_object(
        'lightspeed', tot.n_oms_lightspeed,
        'eitje',      tot.n_oms_eitje,
        'geen',       tot.n_oms_geen
      )
    ),
    'per_vestiging', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        ...
        'omzet_bron_mix', jsonb_build_object(
          'lightspeed', pv.n_oms_lightspeed,
          'eitje',      pv.n_oms_eitje,
          'geen',       pv.n_oms_geen
        )
...
-- 2. Tijdreeks: signature-wijziging → drop + recreate
DROP FUNCTION IF EXISTS public.rpc_cijfers_uren_tijdreeks(text[], date, date, text);
...
BEGIN
  ...
  SELECT
    b.bucket,
    b.vestiging,
    SUM(b.gewerkte_uren)::numeric AS gewerkte_uren,
    SUM(b.geplande_uren)::numeric AS geplande_uren,
    SUM(b.loonkosten)::numeric    AS loonkosten,
