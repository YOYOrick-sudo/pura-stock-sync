CREATE SEQUENCE IF NOT EXISTS public.inkoop_bestelnummer_seq;
  ...
  LOOP
    IF v_ronde.route_type = 'leverancier' THEN
      ...
      WHERE vestiging = p_vestiging AND leverancier_id = v_ronde.leverancier_id
        ...
        AND coalesce(leverdatum, '1900-01-01') = coalesce(v_leverdatum, '1900-01-01')
      LIMIT 1;
      ...
      ELSE
        UPDATE inkoop_orders SET telronde_id = v_ronde.id WHERE id = v_order_id;
...
GRANT EXECUTE ON FUNCTION public.rpc_genereer_bestelvoorstel(text, date) TO authenticated, service_role;