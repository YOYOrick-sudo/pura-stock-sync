-- 1. Eén open telronde per route/dag; afgeronde rondes mogen naast elkaar bestaan
DROP INDEX IF EXISTS telrondes_uniek;
CREATE UNIQUE INDEX telrondes_uniek_open
  ON public.telrondes (vestiging, route_type, COALESCE(leverancier_id::text, bron_vestiging), datum)
  WHERE deleted_at IS NULL AND status = 'open';

-- 2. Voorstel gebruikt alleen de laatste afgeronde telronde per route
CREATE OR REPLACE FUNCTION public.rpc_genereer_bestelvoorstel(p_vestiging text, p_datum date DEFAULT NULL)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  FOR v_ronde IN
    SELECT DISTINCT ON (route_type, coalesce(leverancier_id::text, bron_vestiging)) *
    FROM telrondes
    WHERE vestiging = p_vestiging AND datum = v_datum AND status = 'afgerond' AND deleted_at IS NULL
    ORDER BY route_type, coalesce(leverancier_id::text, bron_vestiging),
             coalesce(afgerond_op, created_at) DESC
  LOOP
    v_order_id := NULL;
    v_leverdatum := NULL;
...
-- 3. Weergavenaam per gebruiker: profielnaam, anders e-mailadres
