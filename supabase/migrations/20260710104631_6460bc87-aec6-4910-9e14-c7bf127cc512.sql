...
RETURNS TABLE(
  vestiging text, werkdag date, isodow int,
  ...
  verspilling numeric, loonkosten_bron text, pct_vangnet numeric,
...
BEGIN
  ...
  ritme AS (
    ...
    FROM bez b LEFT JOIN oms o ON o.vestiging=b.vestiging AND o.werkdag=b.werkdag AND o.uur=b.uur
  ...
  ritme_team AS (
    ...
    FROM bez_team bt WHERE bt.werkdag BETWEEN p_van AND p_tot
  ...
  uur_sig AS (
    SELECT dd.vestiging, dd.werkdag, dd.dag_loon_pct, dd.loon_pct_doel, dd.marge_pp,
           ...
           b.uur::int AS uur, b.headcount, b.fte_fractie,
    ...
    LEFT JOIN vang v ON v.vestiging=b.vestiging AND v.werkdag=b.werkdag AND v.uur_approx=b.uur::int
  ...
  labeled AS (
    SELECT us.*,
           (us.uur - ROW_NUMBER() OVER (PARTITION BY us.vestiging, us.werkdag ORDER BY us.uur))::int AS grp
    FROM uur_sig us
  ),
  clusters AS (
  ...
  SELECT
    ...
    c.uur_van::smallint, c.uur_tot::smallint,
    ROUND(c.dag_loon_pct,1), ROUND(c.doel_pct,1), ROUND(c.marge_pp_out,1),
    ROUND(c.hc_gem,2), ROUND(c.ritme_hc,2),
    ROUND(c.d_hc,2), ROUND(c.d_fte_sum,2),
    ROUND(c.oms_cluster,2), ROUND(c.ritme_oms_cluster,2),
