...
-- Preset-driven vergelijkperiode.
-- Frontend stuurt expliciete mode; datum-heuristiek vervalt.

DROP FUNCTION IF EXISTS public.f_vorige_periode(date, date);

CREATE OR REPLACE FUNCTION public.f_vorige_periode(
  p_van date,
  p_tot date,
...
AS $function$
BEGIN
  -- dag (preset "vandaag"): zelfde weekdag vorige week
  IF p_mode = 'dag' THEN
    prev_van := p_van - 7;
    prev_tot := p_tot - 7;
    RETURN NEXT; RETURN;
  ...
  -- week (preset "week"): week ervoor, weekdag-eerlijk
  IF p_mode = 'week' THEN
    prev_van := p_van - 7;
    prev_tot := p_tot - 7;
    RETURN NEXT; RETURN;
  ...
  -- maand (preset "maand"): zelfde dagen 1 jaar eerder (werkt ook mid-maand)
  IF p_mode = 'maand' THEN
    prev_van := (p_van - INTERVAL '1 year')::date;
    prev_tot := (p_tot - INTERVAL '1 year')::date;
    RETURN NEXT; RETURN;
  END IF;

  -- jaar (preset "jaar"): 1 jaar eerder
  IF p_mode = 'jaar' THEN
    prev_van := (p_van - INTERVAL '1 year')::date;
    prev_tot := (p_tot - INTERVAL '1 year')::date;
    RETURN NEXT; RETURN;
  ...
  -- custom (default): zelfde lengte, direct ervoor, weekdag-eerlijk
  prev_van := p_van - ((p_tot - p_van) + 1);
  prev_tot := p_van - 1;
  RETURN NEXT;
END;
$function$;

-- rpc_cijfers_samenvatting: add p_mode passthrough
CREATE OR REPLACE FUNCTION public.rpc_cijfers_samenvatting(
  p_vestigingen text[], p_van date, p_tot date, p_mode text DEFAULT 'custom'
)
RETURNS jsonb
LANGUAGE plpgsql
STABLE
...
DECLARE
  ...
  v_result jsonb;
BEGIN
  SELECT prev_van, prev_tot INTO v_prev_van, v_prev_tot FROM public.f_vorige_periode(p_van, p_tot, p_mode);
...
-- rpc_cijfers_uren_samenvatting: add p_mode passthrough
