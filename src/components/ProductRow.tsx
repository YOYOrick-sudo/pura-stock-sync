...
export const ProductRow = ({ product, onUpdateStock, refillAmount, isFirst, onEnter, index, onRemove }: ProductRowProps) => {
  ...
  return (
    <tr className="even:bg-muted/30 hover:bg-muted/50 transition-colors border-b border-border">
      ...
      <td className="px-2 py-2.5 sm:px-3 sm:py-3 text-center">
        <span className="font-mono text-sm sm:text-base text-muted-foreground tabular-nums">{product.targetStock}</span>
      </td>
      <td className="px-2 py-2.5 sm:px-3 sm:py-3">
        <div className="flex justify-center">
          <Input
            ...
            data-index={index}
            className="w-20 sm:w-24 text-center text-sm sm:text-base h-9 sm:h-10 px-3 border border-input focus:border-primary focus:ring-primary/20 rounded-lg bg-card"
            autoFocus={isFirst}
          />
        </div>
      </td>
