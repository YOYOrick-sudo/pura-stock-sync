  ...
  const runsQ = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .order('gestart_op', { ascending: false }).limit(5);
      if (error) throw error;
  ...
  return (
    <div className="space-y-4">
          ...
          {(statusQ.data ?? []).map((row) => (
            <div key={row.vestiging} className="bg-card border border-border rounded-[20px] shadow-card p-5">
              <div className="flex items-start justify-between mb-2">
                <div>
                  <div className="text-sm font-semibold">{row.vestiging}</div>
      ...
      <div className="bg-card border border-border rounded-[20px] shadow-card p-5">
        <div className="text-sm font-semibold mb-3">Laatste 5 sync-runs</div>
        {(runsQ.data ?? []).length === 0 ? (
