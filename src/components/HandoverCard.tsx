...
export const HandoverCard = () => {
  ...
  const handleSave = async () => {
    ...
    if (error) { toast.error('Kon overdracht niet opslaan'); console.error(error); return; }
    toast.success(memoText.trim() ? 'Overdracht opgeslagen' : 'Overdracht gewist');
    setIsEditing(false); setMemoText('');
    queryClient.invalidateQueries({ queryKey: ['handover-memo', userLocation] });
  };
  ...
  const handleClear = async () => {
    ...
    if (error) { toast.error('Kon overdracht niet wissen'); console.error(error); return; }
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <h3 style={{ fontFamily: 'Inter, sans-serif', fontSize: '14px', fontWeight: 600, color: '#282E3A' }}>
              Overdracht - Bijzonderheden
            </h3>
          </div>
          <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '12px', color: '#636878', paddingLeft: '26px' }}>
      ...
      {isEditing ? (
        ...
        <>
          <p style={{
            ...
            fontSize: '13px',
            fontWeight: 400,
            color: latestMemo?.message ? '#303542' : '#636878',
          ...
          {latestMemo && latestMemo.message && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
              <Clock size={14} color="#636878" />
              <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '12px', color: '#636878' }}>
